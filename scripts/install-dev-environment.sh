...
DEV_DOMAIN='dev.zetruv.com'
ADMIN_DOMAIN='admin-dev.zetruv.com'
API_DOMAIN='api-dev.zetruv.com'
EXPECTED_IP='103.175.207.127'
DEPLOY_USER='zetruv-deploy'
WRAPPER='/usr/local/bin/deploy-zetruv-frontend-dev'
STAMP=$(date +%Y%m%d%H%M%S)

for cmd in nginx curl dig certbot tar; do
...
check_dns_best_effort() {
  ...
  if [[ "$resolved" == "$EXPECTED_IP" ]]; then
    ...
    echo "WARN: local VPS resolver returned '${resolved:-<empty>}' for $domain; continuing because Certbot will perform the external DNS/HTTP validation."
  fi
}

check_dns_best_effort "$DEV_DOMAIN"
...
curl -fsS --resolve "$API_DOMAIN:443:$EXPECTED_IP" "https://$API_DOMAIN/health" >/dev/null || {
  echo "DEV API is not healthy at https://$API_DOMAIN/health" >&2
  exit 1
}
...
EXPECTED_SHA="${1:-}"
ROOT="/var/www/zetruv-dev"
ADMIN_ROOT="/var/www/zetruv-admin-dev"
DOMAIN="dev.zetruv.com"
ADMIN_DOMAIN="admin-dev.zetruv.com"
API="https://api-dev.zetruv.com"
FORBIDDEN_APIS=("https://api-staging.zetruv.com" "https://api-staging.zetruv.dualangka.com")
...
exec 9>"/tmp/zetruv-frontend-dev.lock"
flock -n 9 || { echo "ERROR: another dev deployment is running"; exit 1; }

BUILD_ROOT="$(mktemp -d /tmp/zetruv-frontend-dev-build.XXXXXX)"
ARCHIVE="${BUILD_ROOT}/source.tar.gz"
cleanup() { rm -rf "$BUILD_ROOT"; }
trap cleanup EXIT

echo "=== DOWNLOAD exact DEV commit ${EXPECTED_SHA} ==="
...
bash scripts/smoke-dev-environment.sh
...
clean_releases "$ROOT" "$PREVIOUS"
clean_releases "$ADMIN_ROOT" "$ADMIN_PREVIOUS"
...
install -d -o "$DEPLOY_USER" -g "$DEPLOY_USER" /var/www/zetruv-dev
install -d -o "$DEPLOY_USER" -g "$DEPLOY_USER" /var/www/zetruv-dev/releases
install -d -o "$DEPLOY_USER" -g "$DEPLOY_USER" /var/www/zetruv-admin-dev
install -d -o "$DEPLOY_USER" -g "$DEPLOY_USER" /var/www/zetruv-admin-dev/releases

STOREFRONT_SITE='/etc/nginx/sites-available/zetruv-frontend-dev-client'
ADMIN_SITE='/etc/nginx/sites-available/zetruv-admin-dev'
...
server {
    ...
    root /var/www/zetruv-dev/current;
    index index.html;
...
server {
    ...
    root /var/www/zetruv-admin-dev/current;
    index index.html;
...
ln -sfn "$STOREFRONT_SITE" /etc/nginx/sites-enabled/zetruv-frontend-dev-client
ln -sfn "$ADMIN_SITE" /etc/nginx/sites-enabled/zetruv-admin-dev

nginx -t
systemctl reload nginx

certbot --nginx -d "$DEV_DOMAIN" --non-interactive --agree-tos --redirect --register-unsafely-without-email
certbot --nginx -d "$ADMIN_DOMAIN" --non-interactive --agree-tos --redirect --register-unsafely-without-email

nginx -t
systemctl reload nginx

echo "PASS: DEV frontend runtime prepared for https://$DEV_DOMAIN and https://$ADMIN_DOMAIN"
echo "DEV deploy wrapper installed: ${WRAPPER}"
