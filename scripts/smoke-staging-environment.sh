...
API_BASE='https://api-staging.zetruv.com/api/v1'
STOREFRONT_URL='https://staging.zetruv.com'

echo '=== STAGING STOREFRONT + ADMIN BUILD ==='
...
echo '4/6 admin points back to STAGING storefront'
grep_bundle dist-admin "$STOREFRONT_URL" || { echo "FAIL: admin bundle does not contain $STOREFRONT_URL" >&2; exit 1; }

echo '5/6 no DEV API leaked into STAGING builds'
for root in dist dist-admin; do
  reject_bundle "$root" 'api-dev.zetruv.com'
  reject_bundle "$root" 'api-dev.zetruv.dualangka.com'
done
...
echo 'PASS: STAGING storefront and admin builds are isolated, independently publishable, and both target api-staging.zetruv.com.'
