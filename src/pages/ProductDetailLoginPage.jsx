...
export default function ProductDetailLoginPage({ slug = 'genshin-impact' }) {
  ...
  useEffect(() => {
    ...
    getCatalogProduct(slug)
      .then((data) => {
        ...
        const preferred = data.variants?.find((item) => (
          item.isAvailable && /980\s*\+\s*110/i.test(item.name || '')
        ))
        const firstAvailable = data.variants?.find((item) => item.isAvailable)
        const first = preferred || firstAvailable || data.variants?.[0]
          ...
          <div className="login-product-body">
            <div className="login-product-container">
              <p className="login-package-description">
                {loading ? 'Loading product…' : error || 'Product is not available.'}
              </p>
            </div>
          </div>
        </main>
  ...
  const publisher = product.game?.publisher || product.game?.name || product.category?.name || 'HoYoverse'
          ...
          <div className="login-product-container login-product-layout">
            ...
            <aside className="login-product-summary">
              ...
              <div className="login-summary-row">
                ...
                <strong>{rupiah(SERVICE_FEE)}</strong>
              </div>
              <div className="login-summary-divider" />
              <div className="login-summary-total">
                <span>Total</span>
